...
impl Window {
    ...
    pub fn paint_glyph(
        ...
        let quantized_x = glyph_origin.x.0 * SUBPIXEL_VARIANTS_X as f32;
        #[cfg(target_os = "macos")]
        let quantized_x = (quantized_x + 0.5).floor();
        #[cfg(not(target_os = "macos"))]
        let quantized_x = round_half_toward_zero(quantized_x);
        let quantized_origin = Point::new(
            quantized_x / SUBPIXEL_VARIANTS_X as f32,
    ...
    pub fn paint_emoji(
        ...
        let scale_factor = self.scale_factor();
        let glyph_origin = origin.scale(scale_factor);
        // Color glyphs need the same horizontal subpixel placement as text.
        // Rounding every emoji to a whole pixel separates it from its run.
        #[cfg(target_os = "macos")]
        let quantized_origin = point(
            (glyph_origin.x.0 * SUBPIXEL_VARIANTS_X as f32 + 0.5).floor()
                / SUBPIXEL_VARIANTS_X as f32,
            round_half_toward_zero(glyph_origin.y.0),
        );
        #[cfg(target_os = "macos")]
        let integer_origin = quantized_origin.map(|c| ScaledPixels(c.floor()));
        #[cfg(not(target_os = "macos"))]
