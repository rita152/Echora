...
impl SyntaxCache {
    fn get_or_insert(&mut self, key: SyntaxKey, build: impl FnOnce() -> Vec<TextRun>) -> Rc<Vec<TextRun>> {
        if let Some(runs) = self.entries.get(&key) { return runs.clone(); }
        let runs = Rc::new(build());
        let bytes = key.0.len() * 2 + std::mem::size_of::<SyntaxKey>() * 2
            + runs.len() * std::mem::size_of::<TextRun>();
        // One exceptional line must not evict the entire working set.
        if bytes > SYNTAX_BYTES { return runs; }
        while self.bytes + bytes > SYNTAX_BYTES || self.entries.len() >= SYNTAX_LINES {
            let Some((old, bytes)) = self.order.pop_front() else { break; };
...
pub(super) enum RowKind {
    ...
    Preview,
    Binary,
    Gap { hunk: usize, count: u32 },
    Context { hunk: usize, number: u32 },
    Code { hunk: usize, left: Option<usize>, right: Option<usize> },
}
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl Row {
    fn same_anchor(self, other: Self) -> bool {
        ...
        match (self.kind, other.kind) {
            (RowKind::Code { hunk, left, right }, RowKind::Code { hunk: other_hunk, left: other_left, right: other_right }) => {
...
impl Default for DiffViewport {
    fn default() -> Self {
        Self {
            key: None, rows: Vec::new(),
            scroll: ListState::new(0, ListAlignment::Top, px(LINE_HEIGHT * 10.0)),
            syntax: RefCell::new(SyntaxCache::default()), mode: None, data: None,
            partners: Vec::new(), width: 0.0, wrap: true, max_line_width: None,
        }
    }
}
impl DiffViewport {
    pub(super) fn syntax_runs(&self, text: &str, language: Option<&'static str>, emphasis: Option<(usize, usize, bool)>, build: impl FnOnce() -> Vec<TextRun>) -> Rc<Vec<TextRun>> {
        self.syntax.borrow_mut().get_or_insert((text.to_owned(), language, emphasis), build)
    }
    pub(super) fn partner(&self, file: usize, hunk: usize, line: usize) -> Option<usize> {
        self.partners.get(file)?.get(hunk)?.get(line).copied().flatten()
    }
    pub(super) fn file_row(&self, file: usize) -> Option<usize> {
        self.rows.iter().position(|row| row.file == file && row.kind == RowKind::Header)
    }
    fn restore(&self, anchor: Option<Row>, offset: gpui::Pixels) {
        let exact = anchor.and_then(|anchor| self.rows.iter().position(|row| anchor.same_anchor(*row)));
...
impl PullRequestsView {
    ...
    pub(in crate::components::pull_requests) fn prepare_diff_viewport(&mut self, window: &mut gpui::Window, cx: &mut gpui::Context<Self>) {
        if self.detail_tab != DetailTab::Code && self.review_tab.is_none() { return; }
        let key = RowsKey {
            generation: self.diff_generation, count: self.diff.len(), loading: self.diff_loading,
            filter: self.tree_filter.read(cx).text().trim().to_lowercase(),
            collapsed: self.collapsed_files.clone(), expanded: self.expanded_context.clone(),
            split: self.split, rich: self.rich,
            contexts: self.file_lines.iter().map(|(path, lines)| (path.clone(), lines.len())).collect(),
            errors: self.file_errors.clone(), inline: self.inline_comment.clone(),
        };
        let data = (self.diff_generation, self.diff.len());
        if self.diff_viewport.data != Some(data) {
            self.diff_viewport.data = Some(data);
            self.diff_viewport.partners = self.diff.iter().map(|file| {
                file.hunks.iter().map(|hunk| {
                    let mut partners = vec![None; hunk.lines.len()];
                    for (left, right) in split_pairs(&hunk.lines) {
                        if let (Some(left), Some(right)) = (left, right) && left != right {
        ...
        if changed {
            ...
            let focus = self.inline_editor.as_ref().map(|editor| gpui::Focusable::focus_handle(editor.read(cx), cx));
            let handles = rows.iter().map(|row| self.row_has_inline(*row).then(|| focus.clone()).flatten()).collect::<Vec<_>>();
            let old_count = self.diff_viewport.scroll.item_count();
            self.diff_viewport.scroll.splice_focusable(0..old_count, handles);
            self.diff_viewport.rows = rows;
            // Source indices from another PR/scope are not valid anchors.
            let same_data = self.diff_viewport.key.as_ref().is_some_and(|old| old.generation == key.generation && old.count == key.count);
            self.diff_viewport.restore(if same_data { anchor } else { None }, top.offset_in_item);
            self.diff_viewport.key = Some(key);
            self.diff_viewport.max_line_width = None;
        }
        let width = self.diff_view_width();
        if !changed && (self.diff_viewport.width != width || self.diff_viewport.wrap != self.wrap || mode_changed) {
        ...
        if self.wrap {
            ...
            for row in &self.diff_viewport.rows {
                ...
                let mut measure = |text: &str| {
                    ...
                    let line = window.text_system().shape_line(text.to_owned().into(), px(12.0), runs.as_slice(), None);
                    maximum = maximum.max(f32::from(line.width()));
                };
                match row.kind {
                    RowKind::Code { hunk, left, right } => {
                        for index in [left, right].into_iter().flatten() { measure(&file.hunks[hunk].lines[index].text); }
                    }
                    RowKind::Context { number, .. } => {
                        if let Some(text) = self.file_lines.get(&file.path).and_then(|lines| lines.get(number.saturating_sub(1) as usize)) { measure(text); }
                    }
                    _ => {}
                }
            }
    ...
    fn build_diff_rows(&self, key: &RowsKey) -> Vec<Row> {
        ...
        for (file_index, file) in self.diff.iter().enumerate() {
            if !key.filter.is_empty() && !file.path.to_lowercase().contains(&key.filter) { continue; }
            let mut push = |kind| rows.push(Row { file: file_index, kind });
            push(RowKind::Header);
            if key.collapsed.contains(&file.path) { continue; }
            if key.rich && file.path.ends_with(".md") && file.status != 'D' { push(RowKind::Preview); continue; }
            if file.binary { push(RowKind::Binary); continue; }
            for (hunk_index, hunk) in file.hunks.iter().enumerate() {
                let start = if hunk_index == 0 { 1 } else { hunk_new_end(&file.hunks[hunk_index - 1].header).map(|end| end.saturating_add(1)).unwrap_or(1) };
                let gap = hunk_new_start(&hunk.header).unwrap_or(1).saturating_sub(start);
                ...
                if key.expanded.contains(&context_key) {
                    if let Some(lines) = self.file_lines.get(&file.path) {
                        for offset in 0..(gap as usize).min(lines.len().saturating_sub(start.saturating_sub(1) as usize)) {
                            push(RowKind::Context { hunk: hunk_index, number: start + offset as u32 });
                        }
                    }
                } else if gap > 0 { push(RowKind::Gap { hunk: hunk_index, count: gap }); }
                if key.split {
                    for (left, right) in split_pairs(&hunk.lines) { push(RowKind::Code { hunk: hunk_index, left, right }); }
                } else {
                    for (index, line) in hunk.lines.iter().enumerate() {
                        let old = line.kind == LineKind::Deleted;
                        push(RowKind::Code { hunk: hunk_index, left: old.then_some(index), right: (!old).then_some(index) });
                    }
                }
            }
        }
    ...
    fn row_has_inline(&self, row: Row) -> bool {
        let Some(inline) = &self.inline_comment else { return false; };
        if self.diff[row.file].path != inline.path { return false; }
        let RowKind::Code { hunk, left, right } = row.kind else { return false; };
    ...
    fn diff_view_width(&self) -> f32 {
        (self.pane_width - if self.file_tree_open { self.tree_width() } else { 0.0 }).max(1.0)
    }
    fn diff_content_width(&self) -> f32 {
        if self.wrap { return self.diff_view_width(); }
        let cell = self.diff_viewport.max_line_width.unwrap_or(0.0);
        (cell * if self.split { 2.0 } else { 1.0 } + if self.split { 1.0 } else { 0.0 }).max(self.diff_view_width())
    }
    pub(super) fn virtual_diff_list(&self, cx: &mut gpui::Context<Self>) -> gpui::AnyElement {
        ...
        }).w(px(self.diff_content_width())).h_full().into_any_element()
    }
    fn render_diff_row(&self, index: usize, cx: &mut gpui::Context<Self>) -> gpui::AnyElement {
        let Some(row) = self.diff_viewport.rows.get(index).copied() else { return div().into_any_element(); };
        let file = &self.diff[row.file];
        let element = match row.kind {
            ...
            RowKind::Binary => div().p(px(16.0)).child("Binary file changed. Open file to view it on GitHub.").into_any_element(),
            RowKind::Gap { hunk, count } => self.hunk_expander(row.file, file, hunk, count, cx).into_any_element(),
            RowKind::Context { hunk, number } => {
                let text = &self.file_lines[&file.path][number.saturating_sub(1) as usize];
                self.context_line(&format!("{}:{hunk}", file.path), number, text, cx).into_any_element()
            }
            RowKind::Code { hunk, left, right } if self.split => {
                ...
                for (line, old) in [(left, true), (right, false)] {
                    ...
                    if let Some(line) = line { cell = cell.child(self.diff_line(row.file, file, hunk, line, old, cx)); }
                    pair = pair.child(cell);
                    if old { pair = pair.child(div().flex_none().w(px(1.0)).bg(self.theme().border)); }
                }
                pair.into_any_element()
            }
            RowKind::Code { hunk, left, right } => {
                let line = right.or(left).expect("code rows contain a source line");
                self.diff_line(row.file, file, hunk, line, left.is_some(), cx).into_any_element()
            }
        };
        ...
        div().id(("pr-virtual-row", index)).w_full().child(element)
            .on_scroll_wheel(cx.listener(|view, event: &gpui::ScrollWheelEvent, _, cx| {
                ...
                if delta.x.abs() > delta.y.abs() || event.modifiers.shift {
                    if !view.wrap {
                        let horizontal = if event.modifiers.shift && delta.y != px(0.0) { delta.y } else { delta.x };
                        let maximum = (view.diff_content_width() - view.diff_view_width()).max(0.0);
                        let x = f32::from(view.diff_scroll.offset().x + horizontal).clamp(-maximum, 0.0);
...
mod tests {
    ...
    fn syntax_cache_reuses_more_than_the_shared_sixteen_line_cache() {
        ...
        for i in 0..100 { cache.get_or_insert((format!("let line_{i} = {i};"), Some("rs"), None), Vec::new); }
        for i in 0..100 { cache.get_or_insert((format!("let line_{i} = {i};"), Some("rs"), None), || panic!("reparsed a warm line")); }
        for i in 0..2000 { cache.get_or_insert((format!("{i}{}", "x".repeat(8192)), None, None), Vec::new); }
    ...
    fn split_and_unified_rows_share_source_anchors() {
        let deleted = Row { file: 2, kind: RowKind::Code { hunk: 3, left: Some(4), right: None } };
        let split = Row { file: 2, kind: RowKind::Code { hunk: 3, left: Some(4), right: Some(5) } };
        assert!(deleted.same_anchor(split));
        assert!(!deleted.same_anchor(Row { file: 7, ..split }));
        let mut viewport = DiffViewport::default();
        viewport.rows = vec![Row { file: 2, kind: RowKind::Header }];
