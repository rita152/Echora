...
impl Render for DiffHarness {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        self.0.update(cx, |view, cx| {
            ...
            div().size_full().flex().flex_col().child(view.diff_surface(cx))
        })
    }
}
...
fn a_large_hunk_only_highlights_the_viewport_and_reuses_warm_runs(cx: &mut TestAppContext) {
    let mut patch = String::from("diff --git a/large.rs b/large.rs\n--- a/large.rs\n+++ b/large.rs\n@@ -1,5000 +1,5000 @@\n");
    for i in 0..5000 { patch.push_str(&format!("-let old_{i} = {i};\n")); }
    for i in 0..5000 { patch.push_str(&format!("+let new_{i} = {i};\n")); }
    patch.push_str("diff --git a/last.rs b/last.rs\n--- a/last.rs\n+++ b/last.rs\n@@ -1 +1 @@\n-old\n+new\n");
    let inner = cx.new(|cx| {
    ...
    let warm = inner.update(&mut visual, |view, _| {
        ...
        assert!(!cache.entries.is_empty(), "the viewport must have a nonzero layout height");
        assert!(cache.entries.len() < 300, "off-screen lines were highlighted: {}", cache.entries.len());
        cache.entries.clone()
    });
    ...
    inner.update(&mut visual, |view, cx| {
        ...
        view.apply_pending_file_scroll(cx);
        assert_eq!(view.diff_viewport.scroll.logical_scroll_top().item_ix, 10001);
