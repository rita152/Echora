...
| **Navigate by prompt** | Jump between a conversation's user prompts from the rail beside the transcript: markers highlight the turns on screen, taper around the pointer, and open a preview that repeats the prompt above the start of the reply. Marking a turn stores a bookmark for this run of the app; the Codex app-server has no bookmark method, so Echora keeps those keys itself. |
