...
| **Import from other agents** | Detect what other coding agents on this machine can contribute, import the selected items, and follow the server's progress and completion notifications. The import page lists each source the backend actually answers for, the items it reported, and the recorded import history. |
